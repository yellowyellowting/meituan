...
for (let i = 0; i < provinceList.length; i++) {
    ...
    province.add(new Option(provinceName));
}

function chooseProvince(pro) {
    selectedProvinceIndex = pro.selectedIndex - 1;
    console.log( selectedProvinceIndex);
    var cityList = provinceList[selectedProvinceIndex].cityList;
    city.innerHTML = `<option value="1">请选择市</option>`;
    for (let i = 0; i < cityList.length; i++) {
        city.add(new Option(cityList[i].name));
    }
}

function chooseCity(city) {
    selectedCityIndex = city.selectedIndex - 1;
    var areaList = provinceList[selectedProvinceIndex].cityList[selectedCityIndex].areaList;
    area.innerHTML = `<option value="1">请选择区域</option>`;
    for (let i = 0; i < areaList.length; i++) {
        area.add(new Option(areaList[i]));
    }
}